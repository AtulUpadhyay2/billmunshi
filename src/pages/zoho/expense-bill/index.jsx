...
const ZohoExpenseBill = () => {
  ...
  const {
    ...
    refetch,
    isFetching,
  } = useGetZohoExpenseBills(getQueryParams());

  // Fetch counts for all tabs
  const { data: allBillsData } = useGetZohoExpenseBills({
    organizationId: selectedOrganization?.id,
  });
  const { data: draftBillsData } = useGetZohoExpenseBills({
    organizationId: selectedOrganization?.id,
    status: "draft",
  });
  const { data: analysedBillsData } = useGetZohoExpenseBills({
    organizationId: selectedOrganization?.id,
    status: "analysed",
  });
  const { data: syncedBillsData } = useGetZohoExpenseBills({
    organizationId: selectedOrganization?.id,
    status: "synced",
  });
  ...
  const { mutateAsync: moveExpenseBills } = useMoveExpenseBills();
  ...
  const renderActionButtons = (bill) => {
    ...
    if (status === "Analysed") {
      return (
        <div className="flex gap-2 flex-wrap items-center">
          <button
            onClick={() => navigate(`/zoho/expense-bill/${bill.id}`)}
                      ...
                      <td colSpan="8" className="table-td text-center py-8">
                        <div className="flex flex-col items-center justify-center space-y-3">
                          ...
                          <div className="text-xs text-slate-400">
                            Upload your first expense bill to get started
                          </div>
                        </div>
                      </td>
                    </tr>
                              ...
                              {bill.file && (
                                <button
                                  onClick={() =>
                                    handleViewFile(
                                      bill.file,
                                      bill.billmunshiName || "Expense Bill",
                                    )
                                  }
          ...
          <div className="space-y-6 p-6">
                        ...
                        <button
                          onClick={() => {
                            if (selectedDuplicateBill?.id) {
                              navigate(
                                `/zoho/expense-bill/${selectedDuplicateBill.id}`,
                              );
                        ...
                        <button
                          onClick={() => {
                            ...
                            if (matchedBill?.bill_id) {
                              navigate(
                                `/zoho/expense-bill/${matchedBill.bill_id}`,
                              );
                              setIsDuplicateModalOpen(false);
                            }
                          }}
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-slate-200 dark:border-slate-700">
              <button
                onClick={() => {
                  setIsDuplicateModalOpen(false);
                  navigate(`/zoho/expense-bill/${selectedDuplicateBill.id}`);
                }}
                ...
                <>
                  <button
                    onClick={() => {
                      ...
                      navigate(`/zoho/expense-bill/${selectedExternalBill.id}`);
                    }}
                    className="px-4 py-2 text-sm font-medium text-white bg-green-600 border border-transparent rounded-lg hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition-colors"
                  >
                    Proceed
