        ...
        <div className="space-y-5">
            <Card
                ...
                headerSlot={
                    <button
                        onClick={handleSync}
                        disabled={isGenerating || !selectedOrganization?.id}
                        ...
                        title="Generate zoho token"
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.8} stroke="currentColor" className={`w-4 h-4 transition-transform duration-300 ${isGenerating ? 'animate-spin' : 'group-hover:rotate-180'}`}>
                            ...
                            {isLoading ? (
                                ...
                                <div className="text-center py-8">
                                    <div className="text-slate-500">
                                        ...
                                        <p className="text-sm mt-2">Click "Generate Token" to create your Zoho access credentials</p>
                                    </div>
                                </div>
                            ) : (
                                <div className="p-6 space-y-6">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                        <div className="bg-slate-50 dark:bg-slate-800 rounded-lg p-4">
                                            ...
                                            <p className="text-lg font-semibold text-slate-900 dark:text-white font-mono break-all">{credentials.clientId || 'N/A'}</p>
                                        </div>
                                        <div className="bg-slate-50 dark:bg-slate-800 rounded-lg p-4">
                                            <h3 className="text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Organisation ID</h3>
                                            <p className="text-lg font-semibold text-slate-900 dark:text-white break-words">{credentials.organisationId || 'N/A'}</p>
                                        </div>
                                        <div className="bg-slate-50 dark:bg-slate-800 rounded-lg p-4">
                                            <h3 className="text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Redirect URL</h3>
                                            <p className="text-lg font-semibold text-slate-900 dark:text-white break-all word-wrap overflow-hidden">{credentials.redirectUrl || 'N/A'}</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="border-t border-slate-200 dark:border-slate-700 pt-6">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                            <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg p-4">
                                                ...
                                                <p className="text-sm font-mono text-amber-900 dark:text-amber-100 break-all overflow-hidden">{maskSensitiveData(credentials.clientSecret)}</p>
                                            </div>
                                            <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg p-4">
                                                <h3 className="text-sm font-medium text-amber-800 dark:text-amber-200 mb-1">Access Code</h3>
                                                <p className="text-sm font-mono text-amber-900 dark:text-amber-100 break-all overflow-hidden">{maskSensitiveData(credentials.accessCode)}</p>
                                            </div>
                                            <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-4">
                                                <h3 className="text-sm font-medium text-green-800 dark:text-green-200 mb-1">Access Token</h3>
                                                <p className="text-sm font-mono text-green-900 dark:text-green-100 break-all overflow-hidden">{maskSensitiveData(credentials.accessToken)}</p>
                                            </div>
                                            <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-4">
                                                <h3 className="text-sm font-medium text-green-800 dark:text-green-200 mb-1">Refresh Token</h3>
                                                <p className="text-sm font-mono text-green-900 dark:text-green-100 break-all overflow-hidden">{maskSensitiveData(credentials.refreshToken)}</p>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="border-t border-slate-200 dark:border-slate-700 pt-6">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                            <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
                                                ...
                                                <p className="text-sm text-blue-900 dark:text-blue-100 break-words">{formatDate(credentials.created_at)}</p>
                                            </div>
                                        </div>
                                    </div>
