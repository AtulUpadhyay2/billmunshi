...
const TallyVendorBill = () => {
  ...
  const {
    ...
    refetch,
    isFetching,
  } = useGetTallyVendorBills(getQueryParams(), {
    enabled: !!selectedOrganization?.id,
  });

  // Fetch counts for all tabs
  const { data: allBillsData } = useGetTallyVendorBills(
    { organizationId: selectedOrganization?.id },
    {
      enabled: !!selectedOrganization?.id,
    },
  );
  const { data: draftBillsData } = useGetTallyVendorBills(
    { organizationId: selectedOrganization?.id, status: "draft" },
    {
      enabled: !!selectedOrganization?.id,
    },
  );
  const { data: analysedBillsData } = useGetTallyVendorBills(
    { organizationId: selectedOrganization?.id, status: "analysed" },
    {
      enabled: !!selectedOrganization?.id,
    },
  );
  const { data: syncedBillsData } = useGetTallyVendorBills(
  ...
  const renderActionButtons = (bill) => {
    ...
    if (status === "Draft") {
      ...
      const isDisabled = isAnalyzing || isBackgroundProcessing;
      return (
        <div className="flex gap-2 flex-wrap items-center">
          <button
            ...
            className={`group relative inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-1 transition-all duration-200 ${
              isBackgroundProcessing
                ? "text-yellow-600 bg-yellow-50 border-yellow-200 cursor-not-allowed opacity-90"
                : isAnalyzing
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <svg
                ...
                strokeWidth={1.8}
                stroke="currentColor"
                className={`w-3.5 h-3.5 ${isLoading || pollingInterval ? "animate-spin" : ""}`}
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                              ...
                              {bill.file && (
                                <button
                                  onClick={() =>
                                    handleViewFile(
                                      bill.file,
                                      bill.bill_munshi_name || "Vendor Bill",
                                    )
                                  }
          ...
          <div className="space-y-6 p-6">
                        ...
                        <button
                          onClick={() => {
                            if (selectedDuplicateBill?.id) {
                              navigate(
                                `/tally/vendor-bill/${selectedDuplicateBill.id}`,
                              );
                        ...
                        <button
                          onClick={() => {
                            ...
                            if (matchedBill?.bill_id) {
                              navigate(
                                `/tally/vendor-bill/${matchedBill.bill_id}`,
                              );
                              setIsDuplicateModalOpen(false);
                            }
                          }}
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-slate-200 dark:border-slate-700">
              <button
                onClick={() => {
                  setIsDuplicateModalOpen(false);
                  navigate(`/tally/vendor-bill/${selectedDuplicateBill.id}`);
                }}
                ...
                <>
                  <button
                    onClick={() => {
                      ...
                      navigate(`/tally/vendor-bill/${selectedExternalBill.id}`);
                    }}
                    className="px-4 py-2 text-sm font-medium text-white bg-green-600 border border-transparent rounded-lg hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition-colors"
                  >
                    Proceed
