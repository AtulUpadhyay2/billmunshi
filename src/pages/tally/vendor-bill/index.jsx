...
const TallyVendorBill = () => {
  ...
  const { data: allBillsData } = useGetTallyVendorBills({ organizationId: selectedOrganization?.id }, {
    enabled: !!selectedOrganization?.id,
  });
  const { data: draftBillsData } = useGetTallyVendorBills({ organizationId: selectedOrganization?.id, status: 'draft' }, {
    enabled: !!selectedOrganization?.id,
  });
  const { data: analysedBillsData } = useGetTallyVendorBills({ organizationId: selectedOrganization?.id, status: 'analysed' }, {
    enabled: !!selectedOrganization?.id,
  });
  const { data: syncedBillsData } = useGetTallyVendorBills({ organizationId: selectedOrganization?.id, status: 'synced' }, {
    enabled: !!selectedOrganization?.id,
  });
  ...
  const handleSelectAll = () => {
    const selectableBills = vendorBills.filter(bill => 
  ...
  const handleUpload = async (formData) => {
    try {
      const response = await uploadVendorBills({ organizationId: selectedOrganization?.id, formData });
  ...
  const renderActionButtons = (bill) => {
    ...
    if (status === 'Analysed') {
      return (
        <div className="flex gap-2 flex-wrap items-center">
          <button
            onClick={() => navigate(`/tally/vendor-bill/${bill.id}`)}
                ...
                <thead className="bg-slate-200 dark:bg-slate-700">
                  <tr>
                    <th scope='col' className='table-th w-12'>
                      {vendorBills.some(bill => bill.status === 'Draft' || bill.status === 'Analysed') && (
                        <input
                          type="checkbox"
                          checked={selectedBills.size > 0 && selectedBills.size === vendorBills.filter(bill => bill.status === 'Draft' || bill.status === 'Analysed').length}
                        ...
                        <td className="table-td">
                          <div className="flex flex-col">
                            ...
                            <div className="flex items-center gap-2 mt-1">
                              {bill.file && (
                                <button
                                  onClick={() => handleViewFile(bill.file, bill.bill_munshi_name || 'Vendor Bill')}
                        ...
                        <td className="table-td">
                          <div className="flex gap-2 items-center">
                            {['Analysed', 'Verified', 'Posted', 'Synced'].includes(bill.status) && (
                              <button
                                onClick={() => {
                                  navigate(`/tally/vendor-bill/${bill.id}`);
                                }}
        ...
        <div className="space-y-4 p-6">
          <div className="text-center">
            ...
            <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
              Selected bills will be moved to Expense Bill
            </p>
          </div>
