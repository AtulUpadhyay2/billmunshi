...
const CopyButton = ({ value, label = "value" }) => {
  ...
  return (
    <button
      type="button"
      onClick={handleCopy}
      className="shrink-0 inline-flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-semibold text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 hover:border-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-md transition-all cursor-pointer"
    >
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <Icon icon="heroicons:arrow-down-tray" className="text-base" />
            Download TCP
            {helpData?.tcp_version && (
              <span className="inline-flex items-center px-1.5 py-0.5 -mr-1 ml-1 rounded-md bg-white/20 text-white text-[10px] font-bold tracking-wide">
      ...
      {isLoading ? (
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-16 text-center">
          <div className="w-16 h-16 rounded-2xl bg-blue-50 dark:bg-blue-950/40 ring-1 ring-blue-100 dark:ring-blue-900/60 text-blue-700 dark:text-blue-400 flex items-center justify-center mx-auto mb-4">
            <Icon icon="heroicons:document-text" className="text-3xl" />
          </div>
              ...
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 flex flex-col">
                <div className="flex items-center gap-3 mb-4 pb-3 border-b border-slate-100 dark:border-slate-800">
                  <span className="shrink-0 w-9 h-9 inline-flex items-center justify-center rounded-md bg-blue-50 dark:bg-blue-950/40 text-blue-700 dark:text-blue-400 ring-1 ring-blue-100 dark:ring-blue-900/60">
                    <Icon icon="heroicons:building-office-2" className="text-base" />
                  </span>
                ...
                <dl className="space-y-3.5 flex-1">
                  <div>
                    <dt className="text-[11px] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-1">
                      Organization name
                    </dt>
              ...
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 flex flex-col">
                <div className="flex items-center justify-between gap-3 mb-4 pb-3 border-b border-slate-100 dark:border-slate-800">
                  <div className="flex items-center gap-3 min-w-0">
                    <span className="shrink-0 w-9 h-9 inline-flex items-center justify-center rounded-md bg-blue-50 dark:bg-blue-950/40 text-blue-700 dark:text-blue-400 ring-1 ring-blue-100 dark:ring-blue-900/60">
                      <Icon icon="heroicons:cog-6-tooth" className="text-base" />
                    </span>
                    <h3 className="text-sm font-bold text-slate-900 dark:text-white tracking-tight">
                      Tally integration
                    </h3>
                ...
                <div className="space-y-3.5 flex-1">
                  {helpData.api_key && (
                    <div>
                      <div className="text-[11px] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-1.5 flex items-center gap-1.5">
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5">
            ...
            {isLoadingGuide ? (
              ...
              <div className="text-center py-10">
                <div className="w-12 h-12 rounded-2xl bg-rose-50 dark:bg-rose-950/40 ring-1 ring-rose-100 dark:ring-rose-900/60 text-rose-600 dark:text-rose-400 flex items-center justify-center mx-auto mb-3">
                ...
                <button
                  ...
                  className="mt-3 inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-slate-900 dark:text-white bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-lg cursor-pointer"
                >
              ...
              <div className="text-center py-10">
                <div className="w-12 h-12 rounded-2xl bg-slate-50 dark:bg-slate-900/60 ring-1 ring-slate-200 dark:ring-slate-800 text-slate-400 dark:text-slate-500 flex items-center justify-center mx-auto mb-3">
              ...
              <ol className="relative space-y-6">
                ...
                <span
                  aria-hidden="true"
                  className="absolute left-3.75 top-2 bottom-2 w-px bg-slate-200 dark:bg-slate-800"
                />
                {setupSteps.map((step, i) => (
                  <li key={step.id || i} className="relative flex gap-4">
                    <span className="relative z-10 shrink-0 w-8 h-8 rounded-full bg-blue-50 dark:bg-blue-950/40 text-blue-700 dark:text-blue-400 ring-4 ring-white dark:ring-slate-900 flex items-center justify-center text-xs font-bold">
                      {step.step_number}
                    </span>
                    <div className="flex-1 min-w-0 pb-1">
                      ...
                      {step.description && (
                        <p className="mt-1 text-[13px] text-slate-600 dark:text-slate-400 leading-relaxed whitespace-pre-line">
                          {step.description}
                        </p>
                      )}
                      {step.image_url && (
                        <div className="mt-3 rounded-xl overflow-hidden border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/60">
                          <img
                            src={step.image_url}
