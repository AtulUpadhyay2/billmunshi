...
export const useGetZohoExpenseBills = (
  ...
  return useQuery({
    ...
    queryKey: [
      "zohoExpenseBills", organizationId, status, page || 1, pageSize || null, search || "",
    ],
    queryFn: () => {
      ...
      return apiFetch(`zoho/org/${organizationId}/expense-bills/${query ? `?${query}` : ""}`);
    },
 ...
 */
export const useDownloadZohoExpenseReport = () =>
  useMutation({
    mutationFn: async ({ organizationId, status = "", ids }) => {
      ...
      if (ids?.length) {
        return downloadAuthenticatedFile(
          `zoho/org/${organizationId}/expense-bills/report/`,
          { fallbackName: `zoho-expense-bills.xlsx`, method: "post", data: { ids } },
        );
      }
      const params = status ? { status } : undefined;
      return downloadAuthenticatedFile(
        `zoho/org/${organizationId}/expense-bills/report/`,
        { fallbackName: `zoho-expense-bills.xlsx`, params },
      );
    },
  });
